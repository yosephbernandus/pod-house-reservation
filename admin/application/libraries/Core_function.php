...
class Core_function
{
	...
	public function validate_ajax_origin()
	{
		// $CI = &get_instance();

		// $CI->db->select('name,value');
		// $CI->db->where_in('name', array('admin_url', 'site_website'));
		// $arr_setting = $CI->db->get('setting');
		// $arr_setting = $arr_setting->result();
		...
		// }
		// if(isset($_SERVER['HTTP_ORIGIN']) && $_SERVER['HTTP_ORIGIN'] == 'http://www.sulutprov.go.id')
		// {
		// 	$result['site_website'] = $_SERVER['HTTP_ORIGIN'];
		// }
		
		// if(isset($_SERVER['HTTP_ORIGIN']) && ($_SERVER['HTTP_ORIGIN'] == $result['site_website'] || $_SERVER['HTTP_ORIGIN'] == $result['site_website'].'/'))
		// {
		// 	header("Access-Control-Allow-Origin: " . $result['site_website']);
		...
		// if(isset($_SERVER['HTTP_ORIGIN']) && ($_SERVER['HTTP_ORIGIN'] == $result['admin_url'] || $_SERVER['HTTP_ORIGIN'] == $result['admin_url'].'/'))
		// {
		// 	return TRUE;
		// }
		
		// if(substr($result['admin_url'], 7) == $_SERVER['HTTP_HOST'] || substr($result['admin_url'], 8) == $_SERVER['HTTP_HOST'])
		// {
		// 	return TRUE;
		// }
